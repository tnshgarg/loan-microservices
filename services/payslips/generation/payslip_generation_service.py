...
RUPEE_SYMBOL = b'\xe2\x82\xb9'.decode()
FONT_NAME = "Roboto"
...
class PayslipGenerationService:
    ...
    def _create_earning_details(self, can):
        ...
        can.drawString(
            258, 480, RUPEE_SYMBOL + earnings.get('basic', 'N/A'))
        can.drawString(258, 465, RUPEE_SYMBOL + earnings.get("hra", "N/A"))
        can.drawString(258, 450, RUPEE_SYMBOL +
                       earnings.get("other_allowance", "N/A"))
        can.drawString(258, 415, RUPEE_SYMBOL +
                       earnings.get("total_earnings", "N/A"))

    def _create_deduction_details(self, can):
        deductions = self.deductions
        can.drawString(520, 480, RUPEE_SYMBOL +
                       deductions.get("tax_deducted", "N/A"))
        can.drawString(520, 465, RUPEE_SYMBOL +
                       deductions.get("pf_contribution", "N/A"))
        can.drawString(520, 450, RUPEE_SYMBOL +
                       deductions.get("professional_tax", "N/A"))
        can.drawString(520, 435, RUPEE_SYMBOL +
                       deductions.get("other_deductions", "N/A"))
        can.drawString(520, 416, RUPEE_SYMBOL +
                       deductions.get("total_deductions", "N/A"))

    def _create_final_details(self, can):
        final = self.final
        net_pay = convert_to_inr(final.get('net_pay', 'N/A'))
        net_salary_in_words = convert_to_words(int(final.get("net_pay")))
        can.drawString(258, 368, RUPEE_SYMBOL + net_pay)
        can.drawString(258, 353, net_salary_in_words)

    def _create_payslip_overlay(self):
        packet = io.BytesIO()
        pdfmetrics.registerFont(
            TTFont('Roboto', 'resources/Roboto-Regular.ttf'))
